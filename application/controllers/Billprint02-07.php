...
class Billprint extends CI_Controller {
    ...
    public function billPrintPdf()
    {
        ...
        {      
            ...
            $printDate=date("d-m-Y");            
             //$jasperphp->debugsql=true;
            $jasperphp->arrayParameter = array('BillingStyle'=>"'".$billing_style."'",'QuarterMonth'=>$QM,'StudentId'=>'"'.implode(',',$student_id).'"','YearId'=>$yearid ,'CompanyName'=>strtoupper($company->full_name),'CompanyAddress'=>$company->address,'CompanyPhone'=>$company->phone,'ofc_time'=>$company->ofc_time,'SUBREPORT_DIR'=>$subreportfile);
            ...
            $jasperphp->transferDBtoArray($server,$user,$pass,$db,$dbdriver);
           // pre($jasperphp);  exit;
            $jasperphp->outpage('I','BillPrint-'.date('d_m_Y-His'));  

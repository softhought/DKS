...
class Dailybalance extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('dailybalancemodel','dailybalancemodel',TRUE); 
...
public function index()
{
  ...
  $session = $this->session->userdata('user_detail');
	...
	{ 
        $page = "dashboard/daily_balance/daily_balance_view.php";
        $header="";  
        
        $result['memberList'] = $this->dailybalancemodel->getMemberListDataForDailyBalance();
...
public function addDailybalance(){
    ...
    {    
      ...
      //  pre($result['memberList']);exit;
        ...
        $page = "dashboard/daily_balance/daily_balance_process.php";
        $header="";
...
public function dailyBalanceAction() {
 ...
 if($this->session->userdata('user_detail'))
    {
        ...
        $processing_dt=$dataArry['processing_dt'];
        $member=$dataArry['member'];

        if($processing_dt!=""){

                $processing_dt = str_replace('/', '-', $processing_dt);
                $processing_dt = date("Y-m-d",strtotime($processing_dt));
       ...
       if ($member!='') {
          ...
          $this->dailyBalanceProcess($processing_dt,$member);
           ...
           foreach ($result['memberList'] as $memberlist) {
                 ...
                 $this->dailyBalanceProcess($processing_dt,$memberlist->member_id);

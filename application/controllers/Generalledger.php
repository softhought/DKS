...
class Generalledger extends CI_Controller {
	...
	public function pdfGeneralLedger(){
	   ...
	   $frmDate = date("Y-m-d",strtotime(trim($this->input->post('from_date'))));  //modify by anil
	   $toDate = date("Y-m-d",strtotime(trim($this->input->post('to_date'))));     //modify by anil
	  ...
	  $fiscalStartDt = $this->generalmodel->getFiscalStartDt($yearid);
	   ...
	   $result['fromDate'] = date('d-m-Y',strtotime($frmDate));

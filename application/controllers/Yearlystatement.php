...
public function print_yearlystatement(){
       if($this->session->userdata('user_detail'))
        {
            $session = $this->session->userdata('user_detail');
            $company=$session['companyid'];
            ...
            $from_month = $this->input->post('from_month');
            $to_month = $this->input->post('to_month');
            $sel_member = $this->input->post('sel_member_code');
        ...
        $result['billList'] = $this->yearlystatementmodel->getMemberBillMasterData($sel_member,$fromyearmonth,$toyearmonth,$year,$company);
               ...
               $where_member = array('member_master.member_id' => $sel_member );
               $result['memberData'] = $this->commondatamodel->getSingleRowByWhereCls('member_master',$where_member);
         ...
         // pre($result['billList']);exit;
            ...
            $page = "dashboard/yearly_statement/yearly_statement_pdf_view.php"; 
                ...
                $pdf->WriteHTML($html); 
                $output = 'yearlystatementPdf' . date('Y_m_d_H_i_s') . '_.pdf'; 
                $pdf->Output("$output", 'I');
                exit();
         }
         else {

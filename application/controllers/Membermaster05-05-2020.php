...
public function registration_action(){
      ...
      $session = $this->session->userdata('user_detail');
        if($this->session->userdata('user_detail'))
        {
           ...
           $dir = $_SERVER['DOCUMENT_ROOT'].'/DKS/assets/img/member-images';  //server

          $isImage = trim(htmlspecialchars($this->input->post('isImage')));
          $memberId = trim(htmlspecialchars($this->input->post('memberId')));
          $upd_where = array('member_master.member_id' => $memberId);
          ...
          //   $this->upload->initialize($config);
              ...
              if ($this->upload->do_upload('imagefile')) {
               ...
               $image_data = array('image_name'=>$imagefile,'is_image'=>$isImage);
                  ...
                  $Updatedata = $this->commondatamodel->updateSingleTableData('member_master',$image_data,$upd_where);

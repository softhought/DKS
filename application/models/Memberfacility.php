...
public function facilitylist()
{
    ...
    {  
       ...
       $result['memberList'] = $this->memberfacilitymodel->getallmebercode();
       //  $entry_module='All';
         $member_id='All';
...
public function facility_action(){
  ...
  public function getFixedHardCourtListByDate(){
    ...
    $session = $this->session->userdata('user_detail');
      ...
      public function getFixedHardCourtListHistory(){
    ...
    $session = $this->session->userdata('user_detail');
        ...
        $result['fixedhrdcourtList'] = $this->memberfacilitymodel->getExitingmemberFixedHardCourt($sel_day,$sel_day_night,$sel_single_double,$sel_time_slot,$court_no);
       // pre($result['fixedhrdcourtList']);exit;

         $page = "dashboard/member_facility/fixedhardcourt_list_partial_view_history.php"; 
...
function insertFixedHardCourtActivity($description,$old_description,$table_id,$action){
    ...
    public function checkFixedHrdCourtBooked()
    {
        if($this->session->userdata('user_detail'))
        {
            ...
            $tranId=explode(" ",$tranId);
           

            if ($sel_single_double=='SINGLES') {
              $max_book=2;
            }else{
              $max_book=4;
            }
            ...
            if ($mode=='ADD') {
               $result = $this->memberfacilitymodel->getExitingmemberFixedHardCourt($sel_day,$sel_day_night,$sel_single_double,$sel_time_slot,$court_no);
            }else{

               $result = $this->memberfacilitymodel->getExitingmemberFixedHardCourtWithInnoreId($sel_day,$sel_day_night,$sel_single_double,$sel_time_slot,$court_no,$tranId);
           ...
           if ($total_rows>=$max_book) {
               $json_response = array(
                            "msg_status" => 1,
                            "msg" => "This slot already booked by another members",  
                        );
           }else{
                 $json_response = array(
                            "msg_status" => 0,
